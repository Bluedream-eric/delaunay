...
{// on regarde si pk est dans le cercle circonscit au triangle formé par pijr
	...
	if (sqrt((centreX-(PK->x))*(centreX-(PK->x)) + (centreY-(PK->y))*(centreY-(PK->y))) >=0.5*Rayon)
	{	
...
{
return ( fabs( triArea(*(triangle->sommet0),*(triangle->sommet1),*(triangle->sommet2)) - ( triArea(*point,*(triangle->sommet0),*(triangle->sommet1)) + triArea(*point,*(triangle->sommet1),*(triangle->sommet2))+ triArea(*point,*(triangle->sommet0),*(triangle->sommet2))  )    )  <= 0.0000001 );
